//   multiply two matrix using pthreads
#include<stdio.h>
#include<pthread.h>
#include<unistd.h>
#include<stdlib.h>
...
// thread computes single element in the resultant matrix
void *mult(void* arg)
{
	int *data = (int *)arg;
	int k = 0, i = 0;
...
int MAX1=0,MAX2,MAX3,MAX4;
	...
	for (i = 0; i < r1; i++){
			for (j = 0; j < c1; j++){
				printf("enter the first matrix value [%d][%d]\n",i,j);
                scanf("%d", &matA[i][j]);
			}
	}
	...
	for (i = 0; i < r1; i++)
			for (j = 0; j < c1; j++){
				printf("enter the second matrix value[%d][%d]\n",i,j);
                scanf("%d", &matB[i][j]);
			}
